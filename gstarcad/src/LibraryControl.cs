...
internal sealed class LibraryControl : UserControl
{
    ...
    private void LoadThumbnails(IEnumerable<LibraryItem> items, CancellationToken cancellationToken)
    {
        ...
        foreach (var item in items)
        {
            ...
            var thumbnail = DwgPreviewReader.ReadPreview(item.FilePath);
            if (thumbnail == null)
            {
                continue;
            }
    ...
    private void RenameSelected()
    {
        ...
        var safeName = Path.GetFileNameWithoutExtension(trimmedName) ?? string.Empty;
    ...
    private async void GenerateMissingThumbnails()
    {
        ...
        try
        {
            for (var index = 0; index < candidates.Count; index++)
            {
                ...
                _statusLabel.Text = $"  正在生成缩略图 {index + 1} / {candidates.Count}";
                await Task.Yield();
    ...
    private void RemoveThumbnailCache(string filePath)
    {
        ...
        lock (_thumbnailCacheLock)
        {
            if (_thumbnailCache.TryGetValue(filePath, out var removedImage))
            {
